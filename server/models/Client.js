...
const handler = {
    ...
    sendRoomMessage (content, cb) { // send message in room
        ...
        if (game) {           
            is_watcher = ! this.room.game.isPlayerInGame(this);
            if(is_watcher){
                this.room.broadcast({
                    channel: 'sendBystanderMsg',
                    data: {content},
                    sender: this
                });
            }else{
    ...
    startGame (data, cb) { // game start
        ...
        // 不允许单人开局 xb
        if (room.clientIdList.size == 1){
            this.emitErrorMsg({ cb, content: 'you are not allow to start game with only one participant!' });
            console.log('you are not allow to start game with only one participant!');
            return;
        }
        // game
